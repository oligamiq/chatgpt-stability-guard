...
setTimeout(()=>{{const sh=snap('shell-0'),stale=document.getElementById('stale-marker');window.clearRace={{staleRemainsMounted:stale.isConnected,shellStillOwned:sh.cls.includes('csg-tool-ui')&&structuralZero('shell-0')}};stale.remove()}},1100);
setTimeout(()=>{{const loader=document.createElement('div');loader.id='loader';loader.className='no-scrollbar';loader.setAttribute('role','progressbar');loader.style.cssText='width:320px;height:80px';loader.textContent='Loading app template';document.getElementById('row-0').appendChild(loader)}},1150);
...
setTimeout(()=>{{const rows=[...Array(16)].map((_,i)=>snap(`row-${{i}}`));window.done={{stop:!document.querySelector('button[aria-label="Stop answering" i]'),allShellsOffFlow:[...Array(16)].every((_,i)=>zeroShell(`shell-${{i}}`)),allRowsHistorical:rows.every(r=>r.cls.includes('csg-tool-summary')&&!r.cls.includes('csg-tool-summary-live'))}};const out=document.createElement('pre');out.id='result';out.textContent=JSON.stringify({{active:window.active,clearRace:window.clearRace,appGrow:window.appGrow,nestedAction:window.nestedAction,done:window.done}});document.body.appendChild(out)}},2300);
</script></body></html>'''
def main():
