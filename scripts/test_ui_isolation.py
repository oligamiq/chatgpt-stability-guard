...
<section data-testid="conversation-turn-2" data-turn="assistant">
  <div class="agent-turn">
    ...
    <div id="preview-action-sibling-header" class="mt-2 sm:mt-4"><span role="button"><img alt="Previewer">Previewer</span></div>
    <div id="preview-action-sibling" class="no-scrollbar"><div style="height:37px"><iframe id="preview-action-sibling-frame" title="ui://other-tool/action-preview" src="about:blank" style="width:100%;height:100%"></iframe></div></div>
    <div id="preview-action-sibling-divider" class="bg-token-border-default my-3 h-px w-full"></div>
    <div id="preview-action-card"><button id="preview-action-connect">Connect account</button></div>
...
setTimeout(() => {{
  ...
  const result={{
    ...
    adjacentConnectFailsOpenPreview: states['preview-action-sibling'].previewState!=='hidden' &&
      states['preview-action-sibling'].position!=='absolute' && Number(states['preview-action-sibling'].opacity)!==0 &&
      states['preview-action-sibling'].width>0 && states['preview-action-sibling'].height>0 &&
      physicallyRendered('preview-action-connect') && hit['preview-action-connect'],
