...
const COPY = {
  ja: {
    ...
    settings: {
      ...
      showRecentOnly: ['直近N回の対話だけ表示', '古い対話を非表示・既定OFF'], autoContinueIncomplete: ['未完成なら自動Continue', '指定パターンに一致した最新返答へ Continue を自動送信・既定OFF'],
  ...
  en: {
    ...
    settings: {
      ...
      showRecentOnly: ['Show only recent N exchanges', 'Hide older exchanges · Off by default'], autoContinueIncomplete: ['Auto-Continue on incomplete response', 'Automatically send Continue when the latest response matches the configured pattern · Off by default'],
